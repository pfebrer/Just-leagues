...
export default class Clasifications extends React.Component {
  ...
  componentDidUpdate(previousProps,previousState) {
    ...
    if (this.state.typeOfComp && (!areEqual(previousState.typeOfComp,this.state.typeOfComp) || !areEqual(previousState.ranking,this.state.ranking) || !areEqual(previousState.groupsResults,this.state.groupsResults) || !areEqual(previousState.playerName,this.state.playerName) ) ){
      let compView = this.state.typeOfComp.Groups ? (
        <Groups returnGroups={this.returnGroups} ranking={this.state.ranking} handlePress={this.handlePress}/>
      ) : this.state.typeOfComp.Challenges ? (
        <Challenges ranking={this.state.ranking} wentUp={this.state.wentUp} wentDown={this.state.wentDown} playerName={this.state.playerName} handlePress={this.handlePress}/>
      ) : this.state.typeOfComp.Knockout ? (
        null
      ) : null;
  ...
  handlePress = ({content,typeOfCell,matchPlayers,iGroup,resultsPositions,fromAddMatchModal,fromChallenges,matchResult}) => {
    if (typeOfCell == "playerNameCell") {
      ...
      this.props.navigation.navigate("MatchModal",{matchPlayers,points,addResult:this.addResult,iGroup,resultsPositions,playerName:this.state.playerName})
    } else if (fromChallenges) {
      let points = matchResult
      let iGroup = "Reptes"
      this.props.navigation.navigate("MatchModal",{matchPlayers,points,addResult:this.addResult,iGroup,playerName:this.state.playerName,fromChallenges})
    }
  }
  ...
  render() {
    ...
    let addMatchModal = this.state.addMatchModal ? this.state.admin ? (
      <AdminAddMatchModal toggleAddMatchModal={this.toggleAddMatchModal} ranking={this.state.ranking} playerName={this.state.playerName} groupsResults={this.state.groupsResults} addResult={this.addResult}/>
    ) : (
      <AddMatchModal toggleAddMatchModal={this.toggleAddMatchModal} ranking={this.state.ranking} playerName={this.state.playerName} groupsResults={this.state.groupsResults} handlePlayerPress={this.handlePress}/>
    ) : null;
