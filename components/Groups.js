...
export default class Groups extends React.Component {
    ...
    componentDidMount() {
      ...
      /*this.rankingsRef.doc("squashRanking").set({
        ...
        /*firestore.collection("players").get().then((snapShot) => {
          snapShot.forEach((playerdoc)=>{
            let {playerName} = playerdoc.data();
            let position = this.props.ranking.indexOf(playerName)
            firestore.collection("players").doc(playerdoc.id).set({
    ...
    componentDidUpdate (prevProps,prevState){
        ...
        if ( !areEqual(this.props.ranking,prevProps.ranking) || !areEqual(this.state.groupsResults,prevState.groupsResults)){
            ...
            let ranking = this.props.ranking.map((playerName) => {position+=1; return [position,playerName]});
            ...
            if (groupsResults.length > 0 ){
                ...
                groups.push(<Table key={"Group"+String(iGroup)} group={group} iGroup={iGroup} groupResults={groupResults} handlePress={this.props.handlePress}/>)
    ...
    render() {
      ...
      return (
        <View style={styles.container}>
            <ScrollView style={styles.scrollView} ref={(scroller) => {this.scroller = scroller}} bounces={true}>
